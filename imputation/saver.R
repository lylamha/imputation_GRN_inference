...
data_dir <- "./"
expr_files <- list.files(data_dir, pattern = "^ExpressionData.csv", recursive = TRUE, include.dirs = TRUE)
...
impList <- lapply(expr_files, function(file) {
  ...
  mat <- as.matrix(so_dat@assays$RNA@data)
  
  # run SAVER on 20 cores
  saver_res <- saver(mat, size.factor = 1, ncores = 20, estimates.only = F)
  dat.saver <- as.data.frame(saver_res$estimate)
  
  
  new_file <- gsub(".csv", "_saver_imputed.csv", file)
  new_file <- paste0(data_dir, new_file)
  write.csv(dat.saver, file = new_file, quote=F, row.names = T)
